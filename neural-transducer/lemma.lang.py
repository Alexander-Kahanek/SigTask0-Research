...
# first lets try adding 25 percent (of the source language) more data from another language
add_nwords = m.ceil(trn_source.nwords * percent)

for lang in languages:
    # get full addition languages to merge into training source language
    trn_lang = File(filetype='trn').get_language(lang)
    # merge languages with 10 percent of nno language
    ...
    # create new name
    name = f'{source}+{int(percent*100)}p+{lang}'
